#PBS -N metplus_fv3lamdaxpr
#PBS -j oe
#PBS -o /lfs/h2/emc/ptmp/perry.shafran/output/metplus_fv3lamdaxpr.out
#PBS -e /lfs/h2/emc/ptmp/perry.shafran/output/metplus_fv3lamdaxpr.out
#PBS -q "dev"
#PBS -A VERF-DEV
#PBS -S /bin/bash
...
export cycle=t00z
export MET_PLUS_TMP=/lfs/h2/emc/ptmp/perry.shafran/metplus_fv3lamdaxpr
...
export MET_PLUS_STD=/lfs/h2/emc/ptmp/perry.shafran/metplus_fv3lamdaxpr

mkdir -p $MET_PLUS_STD

export model=fv3lamdaxpr
...
POINT_STAT_CONFIG_FILE ={METPLUS_PARM_BASE}/met_config/PointStatConfig_cam_pr
INIT_SEQ = 0
LEAD_SEQ_MAX = 60
MODEL = ${model1}
...
[filename_templates]
FCST_POINT_STAT_INPUT_TEMPLATE = fv3lamdax.{init?fmt=%Y%m%d}/{init?fmt=%2H}/fv3lam.t{init?fmt=%2H}z.pr.f{lead?fmt=%3H}.grib2
POINT_STAT_OUTPUT_TEMPLATE = ${model}
EOF

run_metplus.py ${MET_PLUS_CONF}/pb2nc_cam_pr.conf -c ${MET_PLUS_CONF}/point_stat_cam.conf -c ${MET_PLUS_TMP}/${model}.conf -c ${MET_PLUS_CONF}/shared.conf -c ${MET_PLUS_TMP}/shared_${model}.conf -c ${MET_PLUS_CONF}/system_cam.conf
...
/gpfs/dell2/emc/verification/save/Perry.Shafran/aws/mv_load_to_aws.sh perry.shafran ${MET_PLUS_TMP}/stat/ ${MET_PLUS_TMP}/load_met_fv3lampr.xml
