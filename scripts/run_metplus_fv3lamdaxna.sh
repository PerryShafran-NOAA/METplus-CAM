#PBS -N metplus_fv3lamdaxna
#PBS -j oe
#PBS -o /lfs/h2/emc/ptmp/perry.shafran/output/metplus_fv3lamdaxna.out
#PBS -e /lfs/h2/emc/ptmp/perry.shafran/output/metplus_fv3lamdaxna.out
#PBS -q "dev"
#PBS -A VERF-DEV
#PBS -S /bin/bash
...
export cycle=t00z
export MET_PLUS_TMP=/lfs/h2/emc/ptmp/perry.shafran/metplus_fv3lamdaxna
...
export MET_PLUS_STD=/lfs/h2/emc/ptmp/perry.shafran/metplus_fv3lamdaxna

mkdir -p $MET_PLUS_STD

export model=fv3lamdaxna
...
cat << EOF > ${model}.conf
[dir]
FCST_POINT_STAT_INPUT_DIR = /lfs/h2/emc/ptmp/emc.lam/com/fv3lamdax/v1.0
...
POINT_STAT_CONFIG_FILE ={METPLUS_PARM_BASE}/met_config/PointStatConfig_cam_na
INIT_SEQ = 0
LEAD_SEQ_MAX = 60
MODEL = ${model1}
...
[filename_templates]
FCST_POINT_STAT_INPUT_TEMPLATE = fv3lamdax.{init?fmt=%Y%m%d}/{init?fmt=%2H}/fv3lam.t{init?fmt=%2H}z.namerica.f{lead?fmt=%3H}.grib2
POINT_STAT_OUTPUT_TEMPLATE = ${model}
EOF

run_metplus.py ${MET_PLUS_CONF}/pb2nc_cam_na.conf -c ${MET_PLUS_CONF}/point_stat_cam.conf -c ${MET_PLUS_TMP}/${model}.conf -c ${MET_PLUS_CONF}/shared.conf -c ${MET_PLUS_TMP}/shared_${model}.conf -c ${MET_PLUS_CONF}/system_cam.conf
