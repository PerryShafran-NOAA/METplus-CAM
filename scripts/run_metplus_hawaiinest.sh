#PBS -N metplus_hawaiinest
#PBS -j oe
#PBS -o /lfs/h2/emc/ptmp/perry.shafran/output/metplus_hawaiinest.out
#PBS -e /lfs/h2/emc/ptmp/perry.shafran/output/metplus_hawaiinest.out
#PBS -q "dev"
#PBS -A VERF-DEV
#PBS -S /bin/bash
...
export cycle=t00z
export MET_PLUS_TMP=/lfs/h2/emc/ptmp/perry.shafran/metplus_hawaiinest
...
export MET_PLUS_STD=/lfs/h2/emc/ptmp/perry.shafran/metplus_hawaiinest

mkdir -p $MET_PLUS_STD

export model=hawaiinest
...
OBS_POINT_STAT_INPUT_DIR = {OUTPUT_BASE}/cam/hawaii_cam
[config]
METPLUS_CONF = {OUTPUT_BASE}/conf/${model}/metplus_final_pb2nc_pointstat.conf
POINT_STAT_CONFIG_FILE ={METPLUS_PARM_BASE}/met_config/PointStatConfig_cam_hi
LEAD_SEQ = begin_end_incr(0,60,1)
MODEL = ${model1}
POINT_STAT_GRID = G250
...
[filename_templates]
FCST_POINT_STAT_INPUT_TEMPLATE = nam.{init?fmt=%Y%m%d}/nam.t{init?fmt=%2H}z.hawaiinest.hiresf{lead?fmt=%2H}.tm00.grib2
POINT_STAT_OUTPUT_TEMPLATE = ${model}
EOF

run_metplus.py -c ${MET_PLUS_CONF}/pb2nc_cam_hi.conf -c ${MET_PLUS_CONF}/point_stat_cam.conf -c ${MET_PLUS_TMP}/${model}.conf -c ${MET_PLUS_CONF}/shared.conf -c ${MET_PLUS_TMP}/shared_${model}.conf -c ${MET_PLUS_CONF}/system_cam.conf
...
cp /gpfs/dell2/ptmp/Perry.Shafran/output/metplus_conusnest $MET_PLUS_STD/metplus_conusnest_$DATE.out
